    ...
    me.init = function() {
      ...
      me.menus = Ext.create('Ext.menu.Menu', {
        items: [
          ...
          @if ($user->hasRoute('auth.user.set.role'))
            {
              text: 'Set Role',
              iconCls: 'icon-user',
              handler: formRoles.show
            },
          @endif
          ...
          @endif
          @if ($user->hasRoute('auth.user.import'))
            '-',
            {
              text: 'Import Data',
              iconCls: 'icon-excel',
              menu: [{
                  ...
                  iconCls: 'icon-cloud',
                  handler: function() {
                    window.location = '{{ route('auth.user.export.excel.format.import') }}';
                  }
                },
                {
                  text: 'Upload File',
      ...
      me.grid = Ext.create('Ext.grid.Panel', {
        ...
        tbar: [{
          ...
          @if ($user->hasRoute('auth.user.export.excel'))
            {
              text: 'Export Data',
              iconCls: 'icon-excel',
              handler: function() {
                ...
                window.location = '{{ route('auth.user.export.excel') }}?' + params.join('&');
              }
            },
          @endif
          '->', {
        ...
        columns: [{
            text: "<img src='{{ asset('images/icons/wifi.png') }}'>",
          ...
          {
            text: "<img src='{{ asset('images/icons/yes.png') }}'>",
            dataIndex: 'is_active',
            width: 35,
          ...
          {
            text: "<img src='{{ asset('images/icons/key.png') }}'>",
            dataIndex: 'is_reset',
            width: 35,
            renderer: function(val, obj, rec) {
