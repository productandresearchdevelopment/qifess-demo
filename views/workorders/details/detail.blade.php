<style>
  ...
  .wo-detail .info-container {
    margin: 10px 0px;
    padding: 15px 15px;
  }

  .wo-detail .container span {
    color: #666;
    font-size: 10px;
    display: block;
    font-weight: 700;
  ...
  .wo-detail .container .circle-photo {
    width: 40px;
    height: 40px;
  ...
  .wo-detail .action-container {
    display: flex;
  }

  .wo-detail .action-container .indicator {
    margin: 0px 10px;
  }

  .wo-detail .action-container .indicator .icon {
    border-radius: 50%;
  ...
  .wo-detail .action-container .indicator .line {
    width: 2px;
    background: #EEE;
  ...
  .wo-detail .action-container .content {
    flex: auto;
    padding: 0px 10px;
    margin-top: -3px;
    margin-bottom: 20px
  ...
  .wo-detail .action-container .indicator .startend {
    background: #eee;
    border-radius: 5px;
    display: inline-block;
    font-size: 10px;
  ...
  .wo-detail .action-detail-container {
    margin-top: 10px;
    display: none
  }

  .wo-detail .toggle-detail {
    *font-weight: normal;
    font-size: 12px;
    color: #666
  }

  .wo-detail .action-detail-container .item {
    margin: 10px 0px
  }

  .wo-detail .action-detail-container .group {
    font-size: 12px;
  ...
  .wo-detail .action-detail-container .option {
  ...
  var DetailWo = function(dom, properties) {
    ...
    let renderTpl = function(data) {
      ...
      if (dataTpl.fieldtech) {
        dataTpl.fieldtech.photo = (dataTpl.fieldtech && dataTpl.fieldtech.photo) ? '{{ route('upload.file') }}/' +
          dataTpl.fieldtech.photo : '{{ asset('images/nouser.png') }}';
      ...
      if (dataTpl.last_action_status.type == 2) dataTpl.last_action.duration = dates.diff(startDate, dataTpl
...
      ) : null;
      ...
      dataTpl.actions.forEach(function(action) {
        ...
        if (action.lat && action.long) {
          action.view_map = String.format(
            '<a style="text-decoration: none; color: #007fff" href="https://www.google.com/maps/search/?q={lat},{long}&z=9" target="_view_map">View Map</a>',
            action
          );
        }
        ...
        action.status.details.forEach(function(detail) {
          ...
          if (val) {
            if (detail.type == "hide") {
              ...
              if (val.value) {
                detailValue +=
                  '<img style="width: 100%; border: 1px solid #eee" src="{{ route('upload.file') }}/' +
                  val.value +
                  '">';
              }
            } else {
            ...
            if (detailValue) {
              ...
              actionDetails.push(
                '<div class="item"><span>' + detailName + ":</span> " + detailValue + "</div>"
              );
            }
          }
        });
      ...
      function createAttachmentAction(action) {
        let attachmentAction = Object.assign({}, action);
        attachmentAction.status = Object.assign({}, action.status);
        attachmentAction.id = Math.random();
        attachmentAction.created_by.name = "Automatic By System";
        attachmentAction.status.name = "ATTACHMENT";
        attachmentAction.status.alias = "ATTACHMENT";
        attachmentAction.created_date = dates.format(action.created_at, "M d, H:i");
